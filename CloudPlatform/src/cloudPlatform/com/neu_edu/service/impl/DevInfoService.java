...
import cloudPlatform.com.neu_edu.service.InDevInfoSet;
 ...
 * @date 2020-07-23
 */
public class DevInfoService implements InDevInfoSet {
    ...
    public void setDevInfoTableViewData(TableView<DeviceInfoTableData> tableView, ObservableList<DeviceInfoTableData> dataList,
        ...
        tc_leaseStatus.setCellValueFactory(cellData -> cellData.getValue().leaseStatusProperty());
        tc_factory.setCellValueFactory(cellData -> cellData.getValue().factoryProperty());

       tableView.setItems(dataList);
    }

    public ObservableList<DeviceInfoTableData> getDevInfoTableViewData() {
        ObservableList<DeviceInfoTableData> diList = FXCollections.observableArrayList();
        for (DeviceInfo di : getDevInfoList()) {
    ...
    public void remote(DeviceInfoTableData dtTD, TableView<DeviceInfoTableData> tv_devInfo) throws IOException {
        List<DeviceInfo> list = getDevInfoList();
        for (DeviceInfo di : list) {
            if (di.getDeviceName().equals(dtTD.getDeviceName())) {
                ...
                tv_devInfo.setItems(getDevInfoTableViewData());
    ...
    public void delete(DeviceInfoTableData diTD, TableView<DeviceInfoTableData> tv_devInfo) throws IOException {
        ...
        if (is) {
            getDevInfoTableViewData().remove(diTD);
            List<DeviceInfo> list = getDevInfoList();
    ...
    public void addComboBoxItems(ComboBox<String> comboBox) throws DataLoadException {
        for (DeviceType dt : devTypeService.getDevTypeList()) {
            comboBox.getItems().add(dt.getDeviceType());
        }
    }
            ...
            if (!simpleUtil.isEmpty(comboBox.getSelectionModel().selectedItemProperty().getValue())) {
                if (!simpleUtil.isEmpty(tf_devSize.getText())) {
                    if (!simpleUtil.isEmpty(tf_factory.getText())) {
                        String devCode = "CFP2020" + (int)Math.random() * 99999;
